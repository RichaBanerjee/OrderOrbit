...
<head>
    ...
    <link href="User_Frontend_Files/lib/animate/animate.min.css" rel="stylesheet">
    <link href="User_Frontend_Files/lib/owlcarousel/assets/owl.carousel.min.css" rel="stylesheet">
    <link href="User_Frontend_Files/lib/tempusdominus/css/tempusdominus-bootstrap-4.min.css" rel="stylesheet" />
   ...
   <style>
    ...
    .table-hover tbody tr:hover {
        ...
        color: #232023; /* White text on hover */
    }
    ...
    .btn-remove {
        ...
        color: #fff;
        transition: all 0.3s ease; /* Smooth transition for button color change */
    }

    .btn-remove {
        background-color: #FF204E; /* Updated red color for remove button */
        border-color: #FF204E;
    }
    ...
    .btn-place-order {
        background-color: #38E54D; /* Updated green color for place order button */
        border-color: #38E54D;
    }
...
<body>
    ...
    <div class="container-xxl bg-white p-0">
      ...
      <div class="container-xxl position-relative p-0">
			<nav
				...
				<div class="collapse navbar-collapse" id="navbarCollapse" style="font-family: 'Heebo','sans-serif';">
					<div class="navbar-nav ms-auto py-0 pe-4">
						<a href="Home.jsp" class="nav-item nav-link">Home</a> <a
							href="UserDashboard.jsp" class="nav-item nav-link">Menu</a>
						<a class="nav-item nav-link active" href="Cart.jsp"><span
							class="badge badge-pill badge-danger "> <%= cartsize %></span><i
							class='fa fa-shopping-cart'></i></a> 
							<a href="ViewPastOrder" class="nav-item nav-link">Orders</a>
							<a class="nav-link"
			...
			<div class="container-xxl py-5 bg-dark hero-header mb-5">
				<div class="container text-center my-5 pt-5 pb-4">
					<h1 class="display-3 text-white mb-3 animated slideInDown">Your Cart Items</h1>
					<nav aria-label="breadcrumb">
						<ol class="breadcrumb justify-content-center text-uppercase">
							<li class="breadcrumb-item"><a href="Home.jsp">Home</a></li>
							<li class="breadcrumb-item text-white active" aria-current="page"><a href="UserDashboard.jsp">Menu</a></li>
						</ol>
					</nav>
				</div>
			</div>
 ...
 <div class="container main">
        ...
        <a href="UserDashboard.jsp" class="btn btn-primary btn-back"><i class="fas fa-arrow-left mr-1"></i> Back to Menu</a>
                ...
                <tbody>
                    ...
                    if (cart != null && !cart.isEmpty()) {
                        ...
                        for (Map.Entry<Integer, Cart> entry : cart.entrySet()) {
                            ...
                            totalAmount += cartItem.getItem().getItemPrice() * cartItem.getQuantity();
                            session.setAttribute("totalAmount", totalAmount);
                    %>
                    <tr>
                        <td><%= cartItem.getItem().getItemName() %></td>
                        <td>₹<%= cartItem.getItem().getItemPrice() %></td>
                        <td><%= cartItem.getQuantity() %></td>
                        <td>₹<%= String.format("%.2f", cartItem.getItem().getItemPrice() * cartItem.getQuantity()) %></td>
                        <td>
                            <form action="removeFromCart" method="post" onsubmit="storeFormData(); return confirm('Are you sure you want to remove this item from your cart?')">
                                <input type="hidden" name="itemId" value="<%= cartItem.getItem().getItemId() %>">
                                <button type="submit" class="btn btn-sm btn-danger btn-remove"><i class="fas fa-trash-alt"></i> Remove</i></button>
                            </form>
                        </td>
                    </tr>
                    <% 
    ...
    <%
        if (cart != null && !cart.isEmpty()) {
    %>
    <a href="placeOrder" class="btn btn-lg btn-success btn-place-order"><i class="fas fa-check"></i> Place Order</a>
    <%
        } else {
    %>
    <button class="btn btn-lg btn-success btn-place-order" disabled><i class="fas fa-check"></i> Place Order</button>
    <%
        }
    %>
</div>
